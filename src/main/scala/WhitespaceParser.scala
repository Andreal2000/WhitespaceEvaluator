...
class WhitespaceParser extends JavaTokenParsers {
  ...
  var stack = Stack[Int]()
  val heap = Map[Int, Int]().withDefault(_ => 0)
  val returnAddress = Stack[Int]()
  val subroutines = Map[String, Int]()
  ...
  def program: Parser[List[() => Any]] = rep(command)
  ...
  def run(commands:List[() => Any]): Unit = {
